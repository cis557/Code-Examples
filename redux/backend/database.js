  ...
  if (err) {
    ...
    (othererr) => {
      if (othererr) {
        ...
        db.run(insert, ['Richarg Wright', 'rw@upenn.edu', 'Sociology']);
        db.run(insert, ['Maya A', 'maya@upenn.edu', 'Math']);
        db.run(insert, ['Stokely', 'sto@upenn.edu', 'History']);
        db.run(insert, ['Makeba', 'makeba@upenn.edu', 'Biology']);
        db.run(insert, ['Fannon', 'frantz@upenn.edu', 'Psychology']);
      }
    });
  }
});
